 ...
 */
export async function collectCursorSamples(
  page: import('@playwright/test').Page,
): Promise<CursorPoint[]> {
  try {
    const samples = await page.evaluate(() => (window as any).__demoflow_cursorSamples ?? [])
    return samples as CursorPoint[]
  } catch {
    return []
  }
}
 ...
 */
export function saveCursorTelemetry(outputDir: string, samples: CursorPoint[]): string {
 ...
 */
export function loadCursorTelemetry(outputDir: string): CursorPoint[] {
  const filePath = join(outputDir, 'cursor.json')
...
export function detectDwellCandidates(
  ...
  const candidates: DwellCandidate[] = []
  let dwellStart = 0
...
export function suggestZoomRegions(
  samples: CursorPoint[],
  options: DwellOptions & { depth?: ZoomDepth; leadInMs?: number; leadOutMs?: number } = {},
): ZoomRegion[] {
