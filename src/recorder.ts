...
export function convertToGif(
  ...
  try {
    ...
    execSync(
      `ffmpeg -i "${mp4Path}" -vf "${scaleFilter},palettegen=stats_mode=diff" -y "${palettePath}"`,
      { stdio: 'pipe' },
    )

    // Pass 2: encode GIF with palette
    execSync(
      `ffmpeg -i "${mp4Path}" -i "${palettePath}" ` +
 ...
 */
export function convertToMp4WithZoom(
  webmPath: string,
  mp4Path: string,
  ...
  const probeResult = execSync(
    `ffprobe -v error -select_streams v:0 -show_entries stream=duration,r_frame_rate -of csv=p=0 "${webmPath}"`,
  ...
  if (pauses && pauses.length > 0) {
    ...
    for (const pause of sorted) {
      if (pause.start > cursor) keeps.push({ start: cursor, end: pause.start.toFixed(3) })
      cursor = pause.end
    }
...
function buildFrameExpression(
  ...
  const keyFrames = new Set<number>([0, totalFrames - 1])
  for (const r of regions) {
    ...
    for (const f of frames) {
      if (f >= 0 && f < totalFrames) keyFrames.add(f)
    }
  }
  ...
  const segments: { startFrame: number; endFrame: number; startVal: number; endVal: number }[] = []
 ...
 */
export function convertToMp4WithSpeed(
  webmPath: string,
  mp4Path: string,
  ...
  const durStr = execSync(
    `ffprobe -v error -show_entries format=duration -of csv=p=0 "${webmPath}"`,
    { stdio: 'pipe', encoding: 'utf-8' },
  ).trim()
  ...
  const keepSegs: KeepSegment[] = []
  let cur = 0
  for (const pause of sortedPauses) {
    if (pause.start > cur) keepSegs.push({ start: cur, end: pause.start })
    cur = pause.end
  }
  ...
  interface SpeedSegment { start: number; end: number; speed: number }
  ...
  for (const keep of keepSegs) {
    ...
    for (const sr of sortedSpeeds) {
      ...
      if (srEnd <= keep.start || srStart >= keep.end) continue
  ...
  execSync(
    `ffmpeg -i "${webmPath}" -filter_complex "${speedFilter}" -map "[out]" -c:v libx264 -preset fast -y "${mp4Path}"`,
    { stdio: 'pipe' },
  )
}
