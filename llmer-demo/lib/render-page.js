 ...
 */
export async function renderWithAnnotations(options) {
    const { inputPath, outputPath, width, height } = options;
    const zoomRegions = options.zoomRegions ?? [];
    const annotations = options.annotations ?? [];
    // Probe source video
    const probeResult = execSync(`ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate,nb_frames,duration -of csv=p=0 "${inputPath}"`, { stdio: 'pipe', encoding: 'utf-8' }).trim();
    ...
    const duration = rawDuration || parseFloat(execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${inputPath}"`, { stdio: 'pipe', encoding: 'utf-8' }).trim()) || 10;
...
<script>
  ...
  window.__renderFrame = async function(frameBase64, zoomTransform, annotationCode) {
    ...
    for (let frameIdx = 0; frameIdx < totalFrames; frameIdx += BATCH_SIZE) {
        ...
        const frameData = execSync(`ffmpeg -ss ${startTime.toFixed(4)} -i "${inputPath}" ` +
            `-frames:v ${batchCount} -f image2pipe -vcodec png -`, { stdio: ['pipe', 'pipe', 'pipe'], maxBuffer: 200 * 1024 * 1024 });
        ...
        for (let i = 0; i < pngFrames.length; i++) {
            ...
            const compositedBase64 = await page.evaluate(({ fb, zt, ac }) => window.__renderFrame(fb, zt, ac), { fb: frameBase64, zt: zoom, ac: annotationCode });
            // Decode and pipe to encoder
            const compositedBuffer = Buffer.from(compositedBase64, 'base64');
