...
import { ZOOM_DEPTH_SCALES, type ZoomDepth, type ZoomFocus, type ZoomRegion } from './types.js'
...
function easeOutScreenStudio(t: number): number {
  return cubicBezier(0.16, 1, 0.3, 1, t)
}

function easeConnectedPan(value: number): number {
...
// ── Connected transitions (smooth pan between adjacent zoom regions) ────────
...
function getConnectedRegionPairs(regions: ZoomRegion[]): ConnectedRegionPair[] {
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    if (next.startMs - curr.endMs > CHAINED_ZOOM_PAN_GAP_MS) continue
    pairs.push({
      currentRegion: curr,
      nextRegion: next,
...
export function findDominantRegion(
  ...
  const active = regions
    .map((region) => {
      ...
      if (outPair && timeMs > outPair.currentRegion.endMs) return { region, strength: 0 }
      ...
      if (inPair && timeMs < inPair.transitionEnd) return { region, strength: 0 }
...
export function computeZoomAtTime(
  ...
  if (regions.length === 0) return { scale: 1, x: 0, y: 0 }

  const { region, strength, blendedScale } = findDominantRegion(regions, timeMs)
  if (!region || strength === 0) return { scale: 1, x: 0, y: 0 }
...
export function generateAutoZoomRegions(
  hits: { timeMs: number; bbox: { x: number; y: number; width: number; height: number } }[],
  viewport: { width: number; height: number },
  options: { depth?: ZoomDepth; durationMs?: number } = {},
): ZoomRegion[] {
